...
pRegistar.addEventListener("click", () => {
  ...
  function chamaModalRegistar() {
    ...
    document.getElementById("btnCancelaRegistar").innerHTML = "Cancel";
  ...
  function validaLogin() {
    ...
    fetch(`${urlBase}/login`, {
      ...
      .then(async (response) => {
        ...
        document.getElementById("statusLogin").innerHTML = "Sucess!";
        content.innerHTML = "";
        content.innerHTML=
  ...
  async function getNews(website) {
    ...
    await fetch(myRequest).then(async function (response) {
      if (!response.ok) {
        content.innerHTML = "Can't show articles right now!";
      } else {
        ...
        if (Object.keys(articles).length == 1) {
          // Só retornou um article, detalhamos
          article = articles[0];
          texto += ` 
            <div>
             <a href="${article.url}"> <h4>${article.title} - ${article.source}</h4></a>
            </div>`;
        } else {
          // Retornou mais de um article
